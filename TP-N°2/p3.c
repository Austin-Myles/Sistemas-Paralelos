...
int main(int argc,char*argv[]){
    ...
    {
        printf("\nError en los par�metros. Usage: ./%s N BS T (N debe ser multiplo de BS)\n", argv[0]);
        exit(1);
    }
    ...
    #pragma omp parallel shared(C, D, CxD) private(i,j,k)
    {
    ...
    #pragma omp parallel shared(A, B, AxB) private(i,j,k) 
    {
        #pragma omp for schedule(dynamic, 2)
        for(i=0; i < N; i+=bs){
            for(j=0; j < N; j+=bs){
