...
int main(int argc,char*argv[]){
    ...
    {
        #pragma omp for schedule(static) reduction(max:maxA) reduction(max:maxB) reduction(+:promA) reduction(+:promB)
        for(i=0; i<N*N; i++){
            if(A[i] > maxA){
                maxA = A[i];
            }
        ...
        //USAR OPTIMIZACIÖN AL IGUAL QUE CON PTHREADS EN LOS INDICES.
